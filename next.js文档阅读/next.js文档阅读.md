...
- 参考文献
  ...
  - > <http://jartto.wang/2018/06/08/nextjs-3/>
    >
    > <http://raoenhui.github.io/nextjs/2018/08/31/Nextjs%E4%B8%AD%E6%96%87%E6%96%87%E6%A1%A3/>
    >
...
最后总结：我觉得所谓服务端渲染就是将可以提前渲染出来的东西都在服务端渲染出来，这个过程包括

生成react组件，生成DOM，操作DOM，样式的调整等等部分不定变量的确定数值，服务端数据获取等等；发送到浏览器的是一串html字符串，等待着浏览器去激活，从而开始客户端的逻辑，这里就和之前我们写SPA的一样了，比如客户端的AJAX，比如客户端的路由跳转，样式转化，DOM变化等等。在看文档/开发的时候要对在写的代码究竟在服务端运行还是在客户端运行要有清晰的认识，明白2端渲染的差异性；

同时参考vue中对于SSR的介绍文档，<https://ssr.vuejs.org/zh/>

对于为什么使用SSR和使用SSR要注意的地方都解释的非常明白。简单的说

为什么使用SSR？一是为了SEO，二是为了内容到达时间，一般来说就是国内常提的首屏时间（或者秒出）。

需要注意的地方：一是明白2端渲染的不同，明白所写代码究竟在哪个端运行；二是由于许多渲染过程在服务端进行，导致了更多的服务端负载，如果流量大的话一定要做好缓存和性能优化措施。

最后，如果不是这个web app都要求SSR，而是部分页面要求秒出，可以使用prerendering预渲染技术，在构建阶段就针对特定页面简单生成静态html文件。